...
function ThrottleStream (opts) {
   ...
   * @default 1638
   */
  this.chunkSize = Math.floor(Math.max(1, this.bpsFn(0, 0) / 10))
 ...
 * @param {Function} fn
 */
Transform.prototype._process = function (chunk, fn) {
  ...
  if (this._bytesLeft === 0) {
    ...
    const expected = Math.ceil(this.bpsFn(elapsedTime, bytes) * elapsedTime)
