...
fastify.register(require('fastify-rate-limit'), {
  ...
  errorMessage: 'This is your custom error message' //default 'Rate limit exceeded, retry in 1 minute'
})
```
...
- `errorMessage` : You can pass a string, as a custom error message which will override the default error message.
...
// add a custom error message for a specific route
fastify.get('/custom', {
  config: {
    rateLimit: {
      max: 3,
