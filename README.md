...
- `global` : indicates if the plugin should apply rate limit to all routes within the encapsulation scope.
...
- `ban`: maximum number of 429 responses to return to a client before returning 403 responses. When the ban limit is exceeded, the context argument that is passed to `errorResponseBuilder` will have its `ban` property set to `true`. This parameter is an in-memory counter and will not work properly in a distributed environment.  **Note:** `0` can also be passed to directly return 403 responses when a client exceeds the limit.
