...
```
You can change the response by providing a callback to `errorMessage`.
...
- `errorMessage`: a function to generate a custom response object. Defaults to `(after, max) => ({statusCode: 429, error: 'Too Many Requests', message: ``Rate limit exceeded, retry in ${after}``})`
