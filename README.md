...
fastify.register(require('fastify-rate-limit'), {
  ...
  displayHeaders: { // default show all the response headers when rate limit is reached
...
- `displayHeaders`: define which headers should be added in the response when the limit is reached. Defaults all the headers will be shown
